...
async function runSceneContext() {
    ...
    // Location card
    // Phase 1D: mood/lighting moved out of the location entity into Tier 2
    // description entities (location_color_scheme, location_sound_profile,
    // lighting_design at scene level). Show realization_status and geography
    // here instead — those are the headline scalar fields on the slimmed
    // location.
    if (d.location) {
        html += `<div class="query-context-card">
            ...
            <dl class="query-dl">
                ...
                <dt>Realization</dt><dd>${esc(d.location.realization_status) || '\u2014'}</dd>
                <dt>Geography</dt><dd>${esc(d.location.geography) || '\u2014'}</dd>
            </dl>
        </div>`;
    }
